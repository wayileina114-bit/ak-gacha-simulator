...
function openSelModal(b){
  ...
  h.push('<div class="notice">'+(b.type==='zjselect'?'中坚甄选：选 <b>2</b> 位6★ + <b>3</b> 位5★':'定向甄选：选 <b>1-3</b> 位6★ + <b>1-3</b> 位5★')+' · 切换选择后，该选择的保底与寻访记录独立计算；切回原选择自动恢复</div>');
  ...
  var sc=$('selConfirm'); if(sc)sc.onclick=function(){
    var s2=getSel(b);
    if(s2.six.length<minSel(b,6)){ toast('请至少选择 '+minSel(b,6)+' 位6★干员'); return; }
    if(s2.five.length<minSel(b,5)){ toast('请至少选择 '+minSel(b,5)+' 位5★干员'); return; }
    b._pool=null;
    save();
    closeModal();
  ...
  var sels=$('mBody').querySelectorAll('.selop');
  for(var si7=0;si7<sels.length;si7++){
    (function(el){
      el.onclick=function(){
        var b2=bannerById(state.cur), rar=+el.getAttribute('data-rar'), op=el.getAttribute('data-op');
        var s3=getSel(b2), key=rar===6?'six':'five', max=selMax(b2,rar), minN=minSel(b2,rar);
        var cur=s3[key];
        if(cur.indexOf(op)>=0){
...
function bannerInfoHtml(b){
  ...
  if(isSelect(b)){
    var sB=getSel(b);
    if(!sB.six.length){
      h.push('<div class="notice">🎯 该卡池需先选择 UP 干员才能抽卡（选择后保底/记录独立计算）</div>');
      h.push('<button class="mini-btn" id="btnOpenSel" style="margin:4px auto;display:block">🎯 开始选择UP</button>');
    } else {
      h.push('<div class="rateup">');
      var s6o,si6b;
...
function doPull(n){
  ...
  var b=bannerById(state.cur);
  if(!b)return;
  if(isSelect(b)&&!getSel(b).six.length){ toast('请先选择UP干员'); openSelModal(b); return; }
...
function openModal(opName){
  ...
  h.push('<div class="mhead"><div class="mart" style="cursor:zoom-in"><img loading="lazy" id="martImg" src="'+esc(opArtT(o))+'" data-a="'+esc(o.art||'')+'" data-b="'+esc(avUrl(o))+'" onerror="this.onerror=null;this.src=this.dataset.fb" data-fb="'+esc(opArtT(o))+'" alt=""/><button class="mini-btn artToggle" id="artToggle" style="margin-top:4px;width:100%">🔄 切换立绘</button></div><div class="minfo">');
