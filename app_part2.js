...
function simRunOnce(b, n){
  var st={fails:0,batch:[]}, c6=0,c5=0,gaps=[],trig=0,up6=0,up5=0,i,j2,rar,op;
  for(i=0;i<n;i++){
    var p6=Math.min(0.02+Math.max(0,st.fails-49)*0.02,1);
    if(st.batch.length===9){
      var has5=false;
    ...
    if(rar===6){ c6++; if(st.fails>=99)trig++; gaps.push(st.fails+1); st.fails=0; op=pick6(b); if(isUp6(b,op))up6++; }
    else if(rar===5){ c5++; op=pick5(b); if(isUp5(b,op))up5++; }
    else st.fails++;
    st.batch.push(rar);
    if(st.batch.length===10)st.batch=[];
  }
...
function runSim(b, n){
  var r=simRunOnce(b,n), c6=r.c6, c5=r.c5, gaps=r.gaps, trig=r.trig, up6=r.up6, up5=r.up5;
  var i;
